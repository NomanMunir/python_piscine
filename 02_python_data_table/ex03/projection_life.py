...
def create_scatter_plot(life_df: pd.DataFrame, income_df: pd.DataFrame,
                        year: int):
    """
    ...
    merged_data = pd.merge(life_data, income_data, on='country',
...
def main():
    """
    Load life expectancy and GDP data, then display scatter plot for 1900.
    """
    try:
        life_df = load("../life_expectancy_years.csv")
        income_df = load("../income_per_person_gdppercapita_ppp_"
